...
La riga ha una forma obbligatoria, perché il doctor la legge:

```
| Situazione | `nome-agente` | Modello |
```

Il nome sta fra backtick nella **seconda** colonna. Scritto altrove, quell'agente
risulta `ROSTER_ORPHAN` e la tabella cita un `ROSTER_MISSING` che non esiste.
...
Un progetto non resta dov'è nato: una libreria si fa una demo, uno strumento
diventa un servizio. Il campo sta in `profile` dentro `.claude/framework.json`,
ed è l'unico posto che lo sa.

Non c'è una modalità apposta, e non serve: sono le stesse quattro operazioni
di un'installazione, ripetute sul profilo nuovo.

1. **Roster** — `--activate` per ciò che il campo nuovo implica, `--deactivate`
   per ciò che non serve più. Controlla i conflitti dopo.
2. **Guide** — copia quelle del profilo nuovo, più quelle che gli agenti attivati
   citano (`profile.required_guides`). Le vecchie che nessuno cita più le trova
   il doctor come `SHARED_ORPHAN`.
3. **Cicli** — se il campo nuovo ne dichiara, riassembla la guida del
   coordinatore accodandoli (`assemble.cycle_files`); se ne toglie, riassembla
   senza. Stanno **dentro** la regione kernel: nessun rilievo li vede sparire.
4. **Permessi** — rigenera `.claude/settings.json` da `Profile.settings` del
   profilo nuovo.

Poi aggiorna `profile` in `framework.json`. Saltarlo lascia il progetto a
dichiarare un campo che non ha più: la prossima manutenzione rigenera i permessi
sbagliati, e nessun rilievo può accorgersene — il file dichiara, non verifica.
