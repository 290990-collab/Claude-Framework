...
1. **Parallelismo per ruolo e per costo** — il vincolo nomina ruoli e fasce di costo, mai un modello: un vincolo che nomina un modello muore col modello.
   ...
   - Copie dello stesso agente in parallelo, solo su parti che non si toccano né si influenzano: modello leggero senza limite, intermedio max 3, di punta max 2.
   - Agenti diversi su lavori indipendenti convivono (`architect` sul backend, `frontend` sull'interfaccia, `debugger` su un guasto).
...
9. **Revisione proporzionata, un solo giro:** nessun revisore per le modifiche piccole, uno per un task normale, due isolati per un task importante (§ Il ciclo del codice, passo 5). Il revisore finale **oppure** una skill di review nativa, mai entrambe; le native pesanti solo su richiesta dell'utente. I finding li corregge chi ha il quadro — il coordinatore, o l'agente ancora aperto che ha scritto quel codice — mai uno spawn nuovo, e il coordinatore ne verifica le correzioni coi test. Un rilievo che blocca (sicurezza, perdita di dati) va all'utente, che decide.
