...
def hook_env(**extra: str) -> dict:
    """L'ambiente di Claude Code, non quello di chi lancia i test: senza
    `PYTHONIOENCODING` e senza modalità UTF-8, così un hook che legge stdin
    come testo cade su cp1252 come cadrebbe in uso."""
...
class TestConfigProtection(unittest.TestCase):
    def test_config_protection_blocks_editing_an_existing_lint_config(self):
        """Il nome si confronta in minuscolo (su NTFS `.ESLINTRC.JSON` è lo
        stesso file), il relativo si scioglie contro `cwd` e non contro la
        directory dell'hook, e un percorso non ASCII arriva intatto solo se
        stdin si legge come byte UTF-8."""
    ...
    def test_closed_hooks_block_on_input_they_cannot_read(self):
        """Ogni ramo che non sa decidere blocca, anche quando il motivo non si
        può stampare: exit 1 o 120 per Claude Code vogliono dire «passa»."""
        with tempfile.TemporaryDirectory() as d:
            prj = Path(d)
            (prj / "città").mkdir()
            ...
            cases = (
                ...
                ("block_no_verify", b"{rotto"),
                ("block_no_verify", {"tool_input": {}}),
                ("block_no_verify", shell('git commit -m "non chiusa')),
            )
            ...
            for name in ("config_protection", "block_no_verify"):
                with self.subTest(name=name, stderr="non scrivibile"):
                    with open(unwritable, "rb") as err:
                        self.assertEqual(run_hook(name, b"{rotto", stderr=err).returncode, 2)
...
class TestGateguard(unittest.TestCase):
    ...
    def test_gateguard_denies_the_first_touch_and_lets_the_retry_through(self):
        """Il secondo tentativo passa anche se nomina il file in un'altra forma:
        senza normalizzare, l'agente verrebbe negato di nuovo sullo stesso file."""
    ...
    def test_gateguard_skips_outside_the_project_and_when_disabled(self):
        outside = self.base / "altrove" / "b.py"
        ...
        # Spento non registra: riacceso, il primo tocco resta da fare.
...
class TestHookSources(unittest.TestCase):
    def test_every_hook_postpones_its_annotations(self):
        """Il primo `python` nel PATH può essere un 3.9 (quello di serie su
        macOS): lì `str | None` in una firma solleva alla definizione, e un
        hook chiuso che non arriva a `main` blocca ogni Edit e ogni Bash.
        Compilato senza ereditare i flag di chi chiama, l'import deve esserci."""
...
class TestInstalledCommand(unittest.TestCase):
    @unittest.skipUnless(shutil.which("sh"), "sh non disponibile")
    def test_the_installed_hook_command_runs_under_sh(self):
        """Il comando vero, eseguito come lo esegue Claude Code: `sh -c`, con
        uno spazio nel percorso del progetto. Uno script che non compila, che
        esce con un codice diverso da 0 e 2 o che manca chiude gli hook chiusi:
        per Claude Code un 1 vuol dire «nessuna obiezione». Gateguard resta
        aperto negli stessi casi."""
        ...
        with tempfile.TemporaryDirectory() as d:
            prj = Path(d) / "progetto con spazio"
            ...
            # Uno 0 arriva solo se lo script ha letto il payload: stdin vuoto
            # darebbe 2 anche qui.
