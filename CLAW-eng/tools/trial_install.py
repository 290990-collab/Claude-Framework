...
DOMAIN = {
    ...
    "silent-failure-hunter": (
        "Errors by exception, never by return value. Intended and documented "
        "fallback: a malformed line is skipped and counted in `--stats`. No "
        "timeouts: only local files are read."
    ),
    "conversation-analyzer": (
        "Mistakes already corrected several times: answering with the content "
        "of `fixtures/big.log` instead of the command that measures it; "
        "renaming a command-line option without warning."
    ),
    "comment-analyzer": (
        "Debt tracked only in the issues, not in `TODO`s. Not to be analysed: "
        "`dist/` and the files generated by `python -m build`."
    ),
}
...
GUIDES = {
    ...
    "core/security-guide.md": (
        "Untrusted input: the content of the logs and the paths from the "
        "command line. No secrets and no authentication; `~/.logtail.toml` "
        "holds no credentials. A path outside the given folder is rejected."
    ),
}
...
def choices() -> tuple[profile.Profile, list[str], list[str], tuple[str, ...]]:
    """The questionnaire's answers: profile, roster, guides, hooks.

    A function and not variables inside `install()`: the installation plan is
    computed from the same choices, and two copies would diverge in silence.
    """
...
def install(out: Path) -> int:
    ...
    # Step 5: the TODO is born with the first real entry and the date. A
    ...
    print(f"installed: {len(roster)} agents, {len(guides)} guides -> {out}")
    return len(roster)
