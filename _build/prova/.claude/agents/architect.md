...
Vincoli non negoziabili: `src/logtail/core/` non importa nulla da `src/logtail/cli/` (la direzione è una sola); il formato del file di configurazione `~/.logtail.toml` è un contratto con gli utenti esistenti; lo streaming deve restare a memoria costante, qualunque sia la dimensione del file.
