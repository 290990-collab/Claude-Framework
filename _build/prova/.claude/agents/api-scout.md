...
Dipendenze: `click` per la riga di comando, `rich` per la resa a terminale, versioni bloccate in `requirements.lock`. `rich` cambia API fra minori con una certa frequenza: verificare sempre nel pacchetto installato sotto `.venv/lib/`, non nella documentazione online.
