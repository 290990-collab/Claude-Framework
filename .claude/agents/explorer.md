...
<!-- FRAMEWORK:KERNEL v1.0.0 sha256:e9336c3e — generato, non modificare a mano -->
## Metodo
...
La logica vera sta in `framework/tools/fwbuild/` (5 moduli) e in
`scripts/transcript.py`. Ma il **prodotto** di questo repo è prosa: `method/`,
`coordinator/`, `agents/`, `shared/` sono contenuto, non documentazione del
codice — cercare lì è cercare nel sorgente.

I moduli di metodo hanno prefisso numerico (`00-`, `10-`, `20-`, `30-`) e
vengono concatenati **in quell'ordine**: per capire cosa finisce in un documento
si guarda l'ordine dei nomi, non i titoli.

I codici di rilievo del doctor sono stringhe maiuscole in `doctor.py`: cercare
`PLACEHOLDER`, `KERNEL_DRIFT`, `COORDINATOR_LEAK` trova sia il check sia la
skill che lo spiega.

Sembrano rilevanti e non lo sono: `_build/prova/` è una **copia generata** da
`trial_install.py` — un match lì non è il sorgente e si rigenera; `sources/` e
`dictionary.txt` sono materiale dell'utente, non codice. Non aprire
`~/.claude/projects/`: sono transcript interi.
