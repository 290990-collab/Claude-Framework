...
<!-- FRAMEWORK:KERNEL v1.0.0 sha256:bddface2 — generato, non modificare a mano -->
## Metodo
...
Vincoli non negoziabili, in ordine di gravità:

1. **Separazione per destinatario.** `method/` è pagato da ogni spawn,
   `coordinator/` solo da chi delega. Spostare contenuto dal secondo al primo
   non è un refactoring: è il difetto che il framework esiste per evitare.
2. **`framework/` non cita nulla fuori da sé** — né `scripts/`, né `docs/`, né
   `UPDATE.md`. È ciò che lo rende distribuibile.
3. **I tetti di parole sono test**, non linee guida: kernel comune <1600,
   guida del coordinatore <2000, e il margine è già stretto.

Vince sempre l'approccio meno invasivo su `method/`: ogni riga aggiunta lì la
paga ogni agente per sempre.

Decisioni chiuse, che non si riaprono senza mandato esplicito: niente RAG,
indicizzazione o servizi (`UPDATE.md`, *Cosa non fare*); `framework+sec`
congelato finché D1 non chiude; nessun `@import` senza averlo prima verificato
sulla versione di Claude Code in uso.
