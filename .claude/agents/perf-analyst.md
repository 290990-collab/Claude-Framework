...
<!-- FRAMEWORK:KERNEL v1.0.0 sha256:83089c65 — generato, non modificare a mano -->
## Metodo
...
**Il tempo di CPU qui non è la risorsa scarsa.** La suite gira in
circa due secondi, il doctor è istantaneo, non c'è nessun servizio né nessun job
lungo. Un profiler non ha praticamente niente da dire.

La risorsa scarsa sono i **token**: il costo in contesto per task. La metrica di
prestazione di questo progetto si misura con `python scripts/transcript.py
--csv`, non con `cProfile`, e il collo di bottiglia candidato è il contesto
caricato a ogni spawn — cioè `CLAUDE.md`.

**Nessuna ottimizzazione è ancora stata scartata**, perché nessun collo di
bottiglia è stato misurato: è esattamente ciò che D1 e l'ablation D2 devono
stabilire. Prima di proporre una potatura, serve il numero.
