...
La suite gira con `cd framework/tools && python -m unittest discover
tests -q` (103 test) e con `python -m pytest -q`, che deve dare lo **stesso**
numero. I test dello strumento di misura stanno a parte: `cd scripts && python
-m unittest test_transcript -q`, 16 test.

I test vivono in `framework/tools/tests/` e in `scripts/test_transcript.py`.

**Escluso per natura:** la qualità del metodo. Nessun test dice se la prosa
funziona — è precisamente ciò che D1 deve misurare, e nel frattempo i 103 test
verificano l'assemblatore, non il metodo. Non spacciarli per evidenza sul metodo.

Difetti già capitati, che meritano una regressione se si ripresentano:
`trial_install.py` copiava `roadmap.md` senza compilarla, e il doctor usciva 1
su `PLACEHOLDER`; `transcript.py` nominava i subagent con l'`agentId` opaco
perché si fermava alla prima riga utile; la regex dei tag autochiusi mangiava
anche quelli di apertura, lasciando passare iniezioni di sistema come richieste
umane.
