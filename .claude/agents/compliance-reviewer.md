...
<!-- FRAMEWORK:KERNEL v1.0.0 sha256:a789cbfd — generato, non modificare a mano -->
## Metodo
...
**Licenza: non ancora dichiarata.** È una decisione aperta, non
una dimenticanza da colmare in autonomia — il progetto è un prodotto potenziale
e la scelta ha conseguenze.

**Nessuna dipendenza**, quindi nessun vincolo di licenza trasmesso: stdlib pura
per vincolo di progetto.

L'unico riferimento esterno è `claude-os` (MIT), fissato a un commit in
`UPDATE.md`. **Non ne è stato copiato codice**: solo osservazioni con `file:riga`
usate come prove nelle direttive. Se un giorno se ne prendesse codice, la
attribuzione MIT diventerebbe dovuta.

**Dati personali**: i transcript di Claude Code contengono conversazioni intere
dell'utente. Restano in locale, non entrano nel repository, e i CSV di
`docs/eval/risultati/` portano **solo numeri** — è una proprietà da preservare,
