...
Lingua: **italiano**, nel codice, nei commenti, nella prosa e nei
messaggi di commit. È una scelta esplicita, non un'abitudine — e D7 prevede di
tradurre tutto in inglese, ma **solo dopo** aver misurato e potato.

Mappa «tipo di contenuto → cartella»: metodo comune in `framework/method/`,
metodo del coordinatore in `framework/coordinator/`, schede degli agenti in
`framework/agents/`, guide in `framework/shared/`, tooling in
`framework/tools/`, strumenti di lavoro non distribuiti in `scripts/`,
documenti di giudizio in `docs/` e `UPDATE.md`.

I moduli di metodo si chiamano con prefisso numerico (`00-`, `10-`, `20-`,
`30-`) perché vengono concatenati in ordine di nome: il prefisso **è**
l'ordine del documento finale.
