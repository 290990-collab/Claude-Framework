...
Python 3.12, **stdlib pura**: nessuna dipendenza, in
`framework/tools/` come in `scripts/`. È un vincolo di prodotto, non una
preferenza — il pacchetto deve installarsi senza chiedere niente.

Type hint moderni (`list[str]`, `Path | None`), `dataclass` per le strutture di
dati, `pathlib` invece di `os.path`, sempre `encoding="utf-8"` esplicito su ogni
lettura e scrittura.

**I docstring spiegano il perché, non il cosa.** È la convenzione più visibile
del codice esistente: ogni modulo apre dicendo quale problema risolve e quale
alternativa è stata scartata. Un docstring che riformula la firma è da togliere.

Nessun formattatore automatico configurato: si segue lo stile del file accanto.
