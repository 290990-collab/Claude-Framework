...
|---|---|---|
| Trovare dove sta qualcosa nel repo | `explorer` | haiku |
| Firme e comportamenti di librerie esterne | `api-scout` | sonnet |
| Tocca >=3 file o un contratto (destinatario, marker, codici del doctor) | `architect` | opus |
| Scrivere o cambiare codice e prosa del framework | `implementer` | opus |
| Estendere la copertura oltre i mini-test dell'implementer | `tester` | sonnet |
| Comportamento osservabile invariato | `refactorer` | opus |
| Un test rosso di causa ignota | `debugger` | opus |
| Un verdetto, una misura o un criterio di eval | `scientific-reviewer` | opus |
| Interpretare i CSV appaiati di D1 | `results-analyst` | opus |
| Revisione finale da zero, senza fidarsi dei report | `final-reviewer` | opus |
| Licenze e vincoli sulle dipendenze — **solo su richiesta** | `compliance-reviewer` | opus |
| Prestazioni — **solo su richiesta** | `perf-analyst` | opus |

`security-reviewer` è nel catalogo ma **non installato**: qui la superficie
critica è una sola, ed è quella scientifica. Si aggiunge con
`framework-sync --activate` se un giorno serve.

## Note di delega per questo progetto

- **Le prove di D1 le lancia l'utente.** Sono sessioni nuove di Claude Code: un
  agente non può produrle. La riga va in *In attesa* con cosa deve riportare.
- **Niente due agenti in parallelo dentro `framework/`.** I tetti di parole sono
  globali sul kernel, non per file: due modifiche indipendenti possono sfondare
  la soglia insieme, e nessuna delle due lo vede da sola.
- **`UPDATE.md` e `docs/` non passano dal ciclo del codice.** Sono documenti di
  giudizio: la revisione che conta è `scientific-reviewer`, non `final-reviewer`.
- **Il doctor prima di dire che un'installazione regge.** Non è una ri-verifica
  ridondante: è l'unica cosa che vede i 14 rilievi.
