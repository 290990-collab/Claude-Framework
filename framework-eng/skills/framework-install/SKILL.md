...
### Conditional — only if the profile makes them relevant
...
`deploy` **or** `infra`, never both · Is the runtime non-trivial (state,
concurrency, I/O)? → `debugger` · Are there heavy operations launched by the
...
**`.claude/framework.json`** — `{"source": ..., "version": "<version>"}`: it is
how `framework-doctor` and `framework-sync` find the source again later. The
path **is not written by you**: `source.reference(<PRJ>, <FW>)` makes it
relative when the source sits inside the project — the first of the three
supported ways — and absolute only when it sits outside. An absolute path to an
internal source is the machine of whoever installed it, and it dies at the
first clone.
...
**State files** — copy the three templates into `docs/` with today's date and
fill in the placeholders **immediately**: the first real entry in `TODO.md`,
the first goal with its criterion in `roadmap.md`. `status.md` is born empty by
construction — you write in it when something closes. It must be done here, not
later: at Step 6 a residual placeholder is a `PLACEHOLDER`, and `TODO.md` is
