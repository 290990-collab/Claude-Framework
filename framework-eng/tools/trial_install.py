...
# Outside `framework/`: the package ships the code that runs the trial, not the trial.
...
# `{{title}}` is a PLACEHOLDER at Step 6.
...
def install(out: Path) -> int:
    ...
    # The path is chosen by `source.reference`: relative when the source sits
    # inside the project, so the file survives the clone.
    ...
    todo.write_text(
        ...
        .replace("{{the active task, one only}}", FIRST_TASK)
    ...
    roadmap.write_text(
        re.sub(
            r"### 1\. \{\{title\}\}.*?(?=## Deliberately out of scope)",
            lambda _: FIRST_GOAL,
